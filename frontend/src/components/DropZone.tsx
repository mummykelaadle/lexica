...
function BookDialog({ isDialogOpen, setIsDialogOpen, fileName, title, setTitle, setCoverImg, handleSubmit }: BookDialogProps) {
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input id="name" value={title} onChange={handleNameChange} className="col-span-3" />
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="cover" className="text-right">
              Cover
            </Label>
            <Input id="cover" name="cover" type="file" accept="image/*" className="col-span-3" onChange={(e) => setCoverImg(e.target.files ? e.target.files[0] : null)} />
          </div>
        </div>
        <DialogFooter>
